...
const translations = {
    ...
    es: {
        ...
        navProcess: "Proceso",
        navLocations: "Locaciones",
        navOpportunities: "Oportunidades",
        navReferences: "Referencias",
        navContact: "Contacto",

        business: "CONTRATAR SERVICIOS",
        professional: "QUIERO SER PARTE",

        businessHeroTitle: "¿Contratar servicios?",
        businessHeroText: "Producción audiovisual de principio a fin.",
        businessHeroButton: "Solicitar cotización",
        businessHeroSecondary: "Ver servicios",

        professionalHeroTitle: "¿Quiero ser parte?",
        professionalHeroText: "Oportunidades, formación y comunidad audiovisual.",
        ...
        locationsTitle: "Locaciones para tu producción",
        locationsSubtitle: "Casas, playas y paisajes de Costa Rica listos para tu próximo rodaje.",
        locationsMore: "Ver todas las locaciones",
        ...
        refCine: "Cine",
        refStudio: "Estudio",
        refProduction: "Producción",
        refTeam: "Equipo",
        refOpenPhone: "Producción: abrir en deleFOCO",
        seeMore: "Ver más",
        openImage: "Ampliar imagen",
        waTitle: "Cuestionario rápido",
        waSubtitle: "Respondé estas preguntas y enviá la información directo por WhatsApp.",
        waName: "Nombre",
        waNamePlaceholder: "Juan Pérez",
        waEmail: "Correo",
        waEmailPlaceholder: "juan@email.com",
        waClientType: "Tipo de cliente",
        waClientPrivate: "Particular",
        waProductionType: "Tipo de producción",
        waLocation: "Locación",
        waLocationPlaceholder: "San José",
        waDate: "Fecha",
        waBudget: "Presupuesto",
        waBudgetPlaceholder: "₡500.000",
        waSend: "Enviar por WhatsApp",
        waRequired: "Por favor completá al menos tu nombre y correo.",
        waMsgIntro: "Hola, quiero enviar la información del cuestionario:",
    ...
    en: {
        ...
        business: "HIRE SERVICES",
        professional: "I WANT TO JOIN",

        businessHeroTitle: "Hire services?",
        businessHeroText: "Audiovisual production from start to finish.",
        businessHeroButton: "Request a quote",
        businessHeroSecondary: "View services",

        professionalHeroTitle: "Want to join?",
        professionalHeroText: "Opportunities, training and an audiovisual community.",
        ...
        locationsTitle: "Locations for your production",
        locationsSubtitle: "Houses, beaches and landscapes across Costa Rica, ready for your next shoot.",
        locationsMore: "See all locations",
        ...
        refCine: "Film",
        refStudio: "Studio",
        refProduction: "Production",
        refTeam: "Team",
        refOpenPhone: "Production: open on deleFOCO",
        seeMore: "See more",
        openImage: "Enlarge image",
        waTitle: "Quick questionnaire",
        waSubtitle: "Answer these questions and send the information directly through WhatsApp.",
        waName: "Name",
        waNamePlaceholder: "John Smith",
        waEmail: "Email",
        waEmailPlaceholder: "john@email.com",
        waClientType: "Client type",
        waClientPrivate: "Individual",
        waProductionType: "Production type",
        waLocation: "Location",
        waLocationPlaceholder: "San José",
        waDate: "Date",
        waBudget: "Budget",
        ...
        waRequired: "Please complete at least your name and email.",
        waMsgIntro: "Hello, I'd like to send the questionnaire information:",
...
const NAV_COLLAPSE_BREAKPOINT = 1360;
...
/* Al cambiar de audiencia, el contenido nuevo debe aparecer ya visible
   (sin sus animaciones de "reveal"), para que la transición se vea limpia. */
function revealIncomingContent() {
...
function swapAudienceContent(audience) {
    setAudience(audience);
    window.scrollTo({ top: 0, behavior: "instant" });
    revealIncomingContent();
}

/* Transición tipo "diapositiva": la vista actual sale hacia un lado
   y la nueva entra desde el otro (empujando), como en Canva.
   Usa la View Transitions API; si el navegador no la soporta,
   hace un deslizamiento equivalente con Web Animations. */
function requestAudienceChange(audience, onDone) {
    ...
    const forward = audience === "professional";

    audienceTransitionRunning = true;
    root.dataset.slideDir = forward ? "forward" : "back";
    root.classList.add("audience-switching");

    const finish = () => {
        root.classList.remove("audience-switching");
    ...
    if (typeof document.startViewTransition === "function") {
        const transition = document.startViewTransition(() => swapAudienceContent(audience));
    ...
    if (!main || !main.animate) {
        swapAudienceContent(audience);
        finish();
        return;
    }

    const shift = forward ? -1 : 1;
    const previousOverflow = main.style.overflowX;
    main.style.overflowX = "clip";

    const slideOut = main.animate(
        [
            { transform: "translateX(0)", opacity: 1 },
            { transform: `translateX(${shift * 12}%)`, opacity: 0 }
        ],
    ...
    slideOut.finished
        ...
        .then(() => {
            slideOut.cancel();
            main.style.overflowX = previousOverflow;
            finish();
        });
}
...
const WHATSAPP_NUMBER = "50661675217";
...
function sendWhatsApp() {
    ...
    const field = name => whatsappForm.elements[name];
    const read = name => getFieldText(field(name));

    const name = read("nombre");
    const email = read("correo");
    const nameField = field("nombre");
    const emailField = field("correo");

    const emailValid = !email || emailField.checkValidity();
    const missing = !name || !email;

    setFieldError(nameField, !name);
    setFieldError(emailField, !email || !emailValid);

    if (missing || !emailValid) {
        showFormMessage(whatsappError, getTranslation(missing ? "waRequired" : "invalidEmail"));
        (!name ? nameField : emailField).focus();
        return;
    }

    hideFormMessage(whatsappError);

    /* 2026-09-25 -> 25/09/2026 */
    const date = read("fecha");
    const prettyDate = date ? date.split("-").reverse().join("/") : "";

    const details = [
        ["waName", name],
        ["waEmail", email],
        ["waClientType", read("cliente")],
        ["waProductionType", read("produccion")],
        ["waLocation", read("locacion")],
        ["waDate", prettyDate],
        ["waBudget", read("presupuesto")]
    ]
        .filter(([, value]) => value)
        .map(([key, value]) => `${getTranslation(key)}: ${value}`);

    const message = [getTranslation("waMsgIntro"), "", ...details].join("\n");
    const url = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`;

    /* Un enlace temporal evita el bloqueo de ventanas emergentes */
...
function isImageModalOpen() {
    return Boolean(imageModal && imageModal.classList.contains("active"));
}
...
function openImageModal(src, alt, trigger) {
    ...
    /* El modal aparece con una transición (visibility): hay que esperar
       a que sea visible para poder darle foco al botón de cerrar */
    window.setTimeout(() => {
        if (isImageModalOpen() && modalClose) modalClose.focus();
    }, 60);
}
...
/* Imágenes sin enlace: se pueden ampliar con clic, Enter o Espacio.
   Las tarjetas con enlace (<a>) navegan y no abren el modal. */
document.querySelectorAll(".reference-card").forEach(item => {
    const img = item.querySelector("img");
    if (!img || item.querySelector("a[href]")) return;

    item.classList.add("is-zoomable");
    item.setAttribute("role", "button");
    item.setAttribute("tabindex", "0");
    item.setAttribute("data-i18n-aria-label", "openImage");

    const open = () => {
        const src = img.currentSrc || img.src;
        if (src) openImageModal(src, img.alt || "", item);
    };
...
document.addEventListener("keydown", event => {
    ...
    if (event.key === "Tab" && isImageModalOpen() && modalClose) {
        event.preventDefault();
        modalClose.focus();
    }
});
...
/* =========================================================
   23B. VIDEO DE REFERENCIAS (SOLO VIDEO)
   - El video llena la tarjeta; no hay imagen de celular.
   - Se pausa cuando no está a la vista para ahorrar batería.
========================================================= */

function setupPhoneVideos() {
    document.querySelectorAll("[data-phone-stage]").forEach(stage => {
        const video = stage.querySelector("video");
        if (!video) return;

        /* Si el video no carga, se oculta para no dejar un recuadro roto */
        video.addEventListener("error", () => stage.classList.add("is-broken"), true);
        ...
        const play = () => {
            ...
            if (attempt && typeof attempt.catch === "function") {
                attempt.catch(() => { /* el navegador bloqueó el autoplay */ });
            }
        };
